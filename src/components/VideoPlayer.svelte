<script>
    ...
    // 🔥🔥🔥 核心：動畫計算邏輯 (共用於 Preview 和 Export) 🔥🔥🔥
    function calculateAnimState(clip, currentTime) {
        let opacity = 1;
        let scale = 1;

        if (!clip) return { opacity, scale };
        ...
        if (clip.animIn && clip.animIn !== 'none') {
            ...
            if (timeInClip < duration) {
                const progress = Math.max(0, timeInClip / duration); // 0 -> 1
                if (clip.animIn === 'fade') opacity = progress;
                if (clip.animIn === 'zoom') scale = progress; // 0 -> 1
            }
        }
        ...
        if (clip.animOut && clip.animOut !== 'none') {
            ...
            if (timeLeft < duration) {
                const progress = Math.max(0, timeLeft / duration); // 1 -> 0
    ...
    // 🔥🔥🔥 即時預覽變數 (Reactive) 🔥🔥🔥
    $: activeClipAnim = activeClip ? calculateAnimState(activeClip, $currentTime) : { opacity: 1, scale: 1 };
    $: activeTextAnim = activeTextClip ? calculateAnimState(activeTextClip, $currentTime) : { opacity: 1, scale: 1 };

    // ... (監聽 export, 類型判斷, Sync, Play/Pause, Loop 保持不變) ...
    // 省略以節省篇幅，請保留原有的程式碼
    $: if ($startExportTrigger > 0) {
        if (!$isExporting && hasClips) { fastExportProcess(); } 
        else if (!hasClips) { console.log("Resetting trigger."); startExportTrigger.set(0); }
    }
    ...
    $: if (!$isExporting && !isSourceMode) {
        if (activeClip) {
            if (isVideoType(activeClip.type)) {
                if (videoRef) {
                    ...
                    if ((!$isPlaying || Math.abs(videoRef.currentTime - seekTime) > 0.25) && !videoRef.seeking) { videoRef.currentTime = seekTime; }
                }
            } else if (isImageType(activeClip.type)) {
                if (imageRef && !imageRef.src.includes(activeClip.fileUrl)) imageRef.src = activeClip.fileUrl;
            }
        } else { if (videoRef) videoRef.removeAttribute('src'); if (imageRef) imageRef.removeAttribute('src'); }
        if (activeAudioClip && audioRef) {
            ...
            if ((!$isPlaying || Math.abs(audioRef.currentTime - audioSeekTime) > 0.25) && !audioRef.seeking) { audioRef.currentTime = audioSeekTime; }
        } else if (audioRef) { if (audioRef.src) audioRef.removeAttribute('src'); }
    }
    $: if (isSourceMode && !$isExporting) {
        const src = $currentVideoSource;
        if (isVideoType(src.type)) { if (videoRef && videoRef.src !== src.url) { videoRef.src = src.url; videoRef.volume = 1.0; videoRef.play().catch(() => {}); } if (audioRef) audioRef.pause(); }
        else if (isAudioType(src.type)) { if (audioRef && audioRef.src !== src.url) { audioRef.src = src.url; audioRef.volume = 1.0; audioRef.play().catch(() => {}); } if (videoRef) { videoRef.pause(); videoRef.removeAttribute('src'); } }
        else if (isImageType(src.type)) { if (imageRef && imageRef.src !== src.url) imageRef.src = src.url; if (videoRef) videoRef.pause(); if (audioRef) audioRef.pause(); }
    }
    $: if (!$isExporting) {
        if ($isPlaying && !isSourceMode) { if (videoRef && activeClip && isVideoType(activeClip.type)) videoRef.play().catch(() => {}); if (audioRef && activeAudioClip) audioRef.play().catch(() => {}); }
        else if (isSourceMode) { } else { if (videoRef) videoRef.pause(); if (audioRef) audioRef.pause(); }
    }
    $: if ($isPlaying && !$isExporting && !isSourceMode) { lastTime = performance.now(); requestAnimationFrame(loop); }
    function togglePlay() {
        if (isSourceMode) { const type = $currentVideoSource.type; if (isVideoType(type)) videoRef.paused ? videoRef.play() : videoRef.pause(); else if (isAudioType(type)) audioRef.paused ? audioRef.play() : audioRef.pause(); return; }
        if (!hasClips || $isExporting) return; if (!$isPlaying && $currentTime >= contentDuration) currentTime.set(0); isPlaying.update(v => !v);
    }
    function loop(timestamp) {
        if (!$isPlaying || $isExporting || isSourceMode) return; if (contentDuration === 0) { isPlaying.set(false); currentTime.set(0); return; }
        const deltaTime = (timestamp - lastTime) / 1000; lastTime = timestamp; currentTime.update(t => t + deltaTime);
        if ($currentTime >= contentDuration) { isPlaying.set(false); currentTime.set(contentDuration); return; } requestAnimationFrame(loop);
    }
    // ... (Helpers, AddToProject, Drag handlers 保持不變) ...
    // ... (請確保 getMediaDuration, calculateAspectRatio, updateETR, getSmartBitrate, mixAllAudio, interleave, convertFloat32ToInt16, addToProject... 都在)
    function getMediaDuration(file, url) { return new Promise((resolve) => { if (file.type.startsWith('image')) { resolve({ duration: 3, width: 1920, height: 1080 }); return; } const isVideo = file.type.startsWith('video') || file.name.toLowerCase().endsWith('.mov'); const element = isVideo ? document.createElement('video') : document.createElement('audio'); element.preload = 'metadata'; element.muted = true; element.src = url; if (isVideo) element.playsInline = true; const isMov = file.name.toLowerCase().endsWith('.mov') || file.type === 'video/quicktime'; let isResolved = false; const timeout = setTimeout(() => { if (isResolved) return; isResolved = true; if (isMov) { alert(`Load Failed: ${file.name}\nTry Chrome.`); resolve(null); } else { console.warn("Timeout, default 30s"); resolve({ duration: 30, width: 1280, height: 720 }); } }, 4000); element.onloadedmetadata = () => { if (isResolved) return; if (element instanceof HTMLVideoElement && (element.videoWidth === 0 || element.videoHeight === 0)) { isResolved = true; clearTimeout(timeout); alert(`Format Error: ${file.name}`); resolve(null); return; } const rawDuration = element.duration; const vWidth = element.videoWidth || 0; const vHeight = element.videoHeight || 0; if (rawDuration !== Infinity && !isNaN(rawDuration) && rawDuration > 0) { isResolved = true; clearTimeout(timeout); resolve({ duration: rawDuration, width: vWidth, height: vHeight }); return; } element.currentTime = 1e7; element.onseeked = () => { if (isResolved) return; isResolved = true; clearTimeout(timeout); let realDuration = element.currentTime; if (realDuration === 0 || realDuration > 360000) if (element.buffered.length > 0) realDuration = element.buffered.end(element.buffered.length - 1); if (realDuration === 0 || realDuration > 360000) realDuration = 30; resolve({ duration: realDuration, width: vWidth, height: vHeight }); }; }; element.onerror = () => { if (isResolved) return; isResolved = true; clearTimeout(timeout); resolve({ duration: 5, width: 0, height: 0 }); }; }); }
    function calculateAspectRatio(w, h) { if (!w || !h) return '16:9'; const ratio = w / h; if (Math.abs(ratio - 16/9) < 0.05) return '16:9'; if (Math.abs(ratio - 9/16) < 0.05) return '9:16'; if (Math.abs(ratio - 1) < 0.05) return '1:1'; if (Math.abs(ratio - 4/5) < 0.05) return '4:5'; return 'custom'; }
    function updateETR(currentTimestamp, totalDuration) { const now = Date.now(); const elapsedRealTime = (now - exportStartTime) / 1000; if (elapsedRealTime < 2 || currentTimestamp <= 0) return "Calculating..."; const processingSpeed = currentTimestamp / elapsedRealTime; const remainingVideoSeconds = totalDuration - currentTimestamp; const secondsLeft = remainingVideoSeconds / processingSpeed; if (!isFinite(secondsLeft) || secondsLeft < 0) return "Calculating..."; if (secondsLeft < 60) return `${Math.ceil(secondsLeft)}s remaining`; const minutes = Math.floor(secondsLeft / 60); const seconds = Math.ceil(secondsLeft % 60); return `${minutes}m ${seconds}s remaining`; }
    function getSmartBitrate(width, height, fps) { const numPixels = width * height; if (numPixels >= 8_294_400) return 80_000_000; if (numPixels >= 3_686_400) return 40_000_000; if (numPixels >= 2_073_600) return 18_000_000; if (numPixels >= 921_600) return 8_000_000; return 4_000_000; }
    function convertFloat32ToInt16(float32Array) { const int16Array = new Int16Array(float32Array.length); for (let i = 0; i < float32Array.length; i++) { const s = Math.max(-1, Math.min(1, float32Array[i])); int16Array[i] = s < 0 ? s * 0x8000 : s * 0x7FFF; } return int16Array; }
    async function mixAllAudio(clips, totalDuration, targetSampleRate) { try { const safeDuration = (totalDuration > 0 && isFinite(totalDuration)) ? totalDuration : 1; const offlineCtx = new OfflineAudioContext(2, Math.ceil(safeDuration * targetSampleRate), targetSampleRate); const promises = clips.map(async (clip) => { try { if (isImageType(clip.type) || clip.type === 'text') return; const response = await fetch(clip.fileUrl); const arrayBuffer = await response.arrayBuffer(); const tempCtx = new AudioContext(); const audioBuffer = await tempCtx.decodeAudioData(arrayBuffer); tempCtx.close(); const source = offlineCtx.createBufferSource(); source.buffer = audioBuffer; const gainNode = offlineCtx.createGain(); gainNode.gain.value = clip.volume !== undefined ? clip.volume : 1.0; source.connect(gainNode); gainNode.connect(offlineCtx.destination); const offset = clip.mediaStartOffset || 0; source.start(clip.startOffset, offset, clip.duration); } catch (e) { } }); await Promise.all(promises); return await offlineCtx.startRendering(); } catch (e) { console.error("Audio Mix Failed:", e); return null; } }
    ...
    function addToProject() { const source = $currentVideoSource; if (!source) return; addToHistory(); const targetStore = isAudioType(source.type) ? audioTrackClips : mainTrackClips; const currentClips = get(targetStore); const insertTime = currentClips.length > 0 ? Math.max(...currentClips.map(c => c.startOffset + c.duration)) : 0; const newClip = { id: generateId(), fileUrl: source.url || source.fileUrl, name: source.name, type: source.type, startOffset: insertTime, duration: source.duration || 5, sourceDuration: isImageType(source.type) ? Infinity : (source.duration || 5), mediaStartOffset: 0, volume: 1.0, file: source.file, thumbnails: source.thumbnails || [], thumbnailUrls: source.thumbnailUrls || [], waveform: source.waveform }; targetStore.update(clips => resolveOverlaps([...clips, newClip], newClip.id)); currentVideoSource.set(null); currentTime.set(insertTime); }
    async function loadSampleProject() { try { isProcessingDrag = true; addToHistory(); const [vidRes, audRes] = await Promise.all([fetch('/sample_video.mp4'), fetch('/sample_audio.mp3')]); if (!vidRes.ok || !audRes.ok) throw new Error(`Sample files missing.`); const vidBlob = await vidRes.blob(); const audBlob = await audRes.blob(); const vidFile = new File([vidBlob], "Demo_Video.mp4", { type: "video/mp4" }); const audFile = new File([audBlob], "Demo_Music.mp3", { type: "audio/mpeg" }); const vidUrl = URL.createObjectURL(vidFile); const info = await getMediaDuration(vidFile, vidUrl); const vidDuration = info ? info.duration : 10; const thumbs = await generateThumbnails(vidFile, vidDuration); const thumbUrls = thumbs.map(b => URL.createObjectURL(b)); const audUrl = URL.createObjectURL(audFile); const audInfo = await getMediaDuration(audFile, audUrl); const audDuration = audInfo ? audInfo.duration : 10; const waveform = await generateWaveform(audFile); const sampleFiles = [ { name: vidFile.name, type: vidFile.type, url: vidUrl, duration: vidDuration, file: vidFile, thumbnails: thumbs, thumbnailUrls: thumbUrls }, { name: audFile.name, type: audFile.type, url: audUrl, duration: audDuration, file: audFile, waveform: waveform } ]; uploadedFiles.update(curr => [...curr, ...sampleFiles]); mainTrackClips.update(clips => [{ id: generateId(), fileUrl: vidUrl, name: vidFile.name, type: vidFile.type, startOffset: 0, duration: vidDuration, sourceDuration: vidDuration, mediaStartOffset: 0, volume: 1.0, file: vidFile, thumbnails: thumbs, thumbnailUrls: thumbUrls }]); audioTrackClips.update(clips => [{ id: generateId(), fileUrl: audUrl, name: audFile.name, type: audFile.type, startOffset: 0, duration: audDuration, sourceDuration: audDuration, mediaStartOffset: 0, volume: 0.5, file: audFile, waveform: waveform }]); textTrackClips.update(clips => [{ id: generateId(), type: 'text', name: 'Text', startOffset: 0, duration: 5, sourceDuration: Infinity, text: "✨FastVideoCutter", fontSize: 28, color: '#ffffff', fontWeight: 'bold', fontFamily: 'Arial, sans-serif', x: 90, y: 85, volume: 1.0, showBackground: true, backgroundColor: '#00000080', strokeWidth: 0, strokeColor: '#000000' }]); projectSettings.update(s => ({ ...s, width: 1280, height: 720, aspectRatio: '16:9' })); if (typeof window !== 'undefined') fetch('/api/discord', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'sample', filename: 'Demo Project', duration: '10' }) }).catch(e => {}); } catch (e) { console.error(e); alert("Failed to load sample project."); } finally { isProcessingDrag = false; } }
    async function handlePreviewDrop(e) { e.preventDefault(); if ($isExporting) return; const jsonString = e.dataTransfer.getData('application/json'); if (jsonString) { try { const data = JSON.parse(jsonString); const storeFile = get(draggedFile); currentVideoSource.set({ ...data, file: storeFile?.file, thumbnails: storeFile?.thumbnails, waveform: storeFile?.waveform }); return; } catch (err) { console.error("Internal drop failed", err); } } const files = Array.from(e.dataTransfer.files); if (files.length === 0) return; isProcessingDrag = true; try { const processedPromises = files.map(async (file) => { if (file.size > 2 * 1024 * 1024 * 1024) { alert(`File too large: ${file.name}`); return null; } const url = URL.createObjectURL(file); const info = await getMediaDuration(file, url); if (!info) return null; const { duration, width, height } = info; const DURATION_LIMIT = 1800; if (duration > DURATION_LIMIT) { if (!confirm(`⚠️ Large File: "${file.name}" > 30 mins.\nProceed?`)) return null; } const thumbnailBlobs = await generateThumbnails(file, duration); const thumbnailUrls = thumbnailBlobs.map(b => URL.createObjectURL(b)); let waveform = null; if (isAudioType(file.type) || isVideoType(file.type)) waveform = await generateWaveform(file); return { name: file.name, type: file.type || 'video/mp4', url: url, duration: duration, width, height, file: file, thumbnails: thumbnailBlobs, waveform: waveform, thumbnailUrls: thumbnailUrls }; }); const results = await Promise.all(processedPromises); const validFiles = results.filter(r => r !== null); const currentClips = get(mainTrackClips); if (currentClips.length === 0 && validFiles.length > 0) { const firstVideo = validFiles.find(f => (isVideoType(f.type) || f.name.endsWith('.mov')) && f.width > 0); if (firstVideo) { addToHistory(); console.log(`[Drag] Auto-set canvas: ${firstVideo.width}x${firstVideo.height}`); projectSettings.update(s => ({ ...s, width: firstVideo.width, height: firstVideo.height, aspectRatio: 'original' })); } } uploadedFiles.update(current => [...current, ...validFiles]); if (validFiles.length > 0) { currentVideoSource.set(validFiles[0]); if (typeof window !== 'undefined') { const firstFile = validFiles[0]; fetch('/api/discord', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'import', filename: firstFile.name, fileCount: validFiles.length, duration: firstFile.duration ? Math.round(firstFile.duration) : 0 }) }).catch(e => {}); } } } catch (err) { console.error("Drop failed:", err); } finally { isProcessingDrag = false; } }
    ...
    function handleDragStart(e) { const source = isSourceMode ? $currentVideoSource : activeClip; if (!source) { e.preventDefault(); return; } if (source.file) draggedFile.set({ file: source.file, thumbnails: source.thumbnails, waveform: source.waveform }); const dragData = JSON.stringify({ url: source.url || source.fileUrl, name: source.name, type: source.type, duration: source.sourceDuration || source.duration || 5, thumbnailUrls: source.thumbnailUrls || [], waveform: source.waveform }); e.dataTransfer.setData('application/json', dragData); e.dataTransfer.effectAllowed = 'copy'; }

    // ------------------------------------------------
    // 🔥🔥🔥 Export Logic (Full + Animation Support) 🔥🔥🔥
    // ------------------------------------------------
    async function fastExportProcess() {
        ...
        try {
            ...
            const muxer = new Muxer({
                ...
                audio: hasAudioSupport ? { codec: audioConfigData.muxerCodec, numberOfChannels: 2, sampleRate: audioConfigData.sampleRate } : undefined,
            ...
            if (hasAudioSupport) {
                ...
                if (mixedBuffer) {
                    ...
                    for (let i = 0; i < totalSamples; i += chunkSize) {
                        ...
                        const audioData = new AudioData({ format: 's16', sampleRate: audioConfigData.sampleRate, numberOfFrames: len, numberOfChannels: 2, timestamp: Math.round((i / audioConfigData.sampleRate) * 1_000_000), data: chunkData });
                        try { audioEncoder.encode(audioData); } catch(e) {}
                        audioData.close();
                    }
                    try { if (audioEncoder.state === 'configured') await audioEncoder.flush(); } catch(e) {}
                }
            }
            ...
            for (let i = 0; i < totalFrames; i++) {
                ...
                if (activeClip) {
                    ...
                    if (sourceElement && sw && sh) {
                        ...
                        ctx.save();
                        ctx.globalAlpha = anim.opacity; // Fade
                        ctx.translate(cx, cy); 
                ...
                if (activeText) {
                    const anim = calculateAnimState(activeText, timeInSeconds); // 🔥 Text Animation
    ...
    async function chooseAudioEncoderConfig() {
        ...
        // Return null if not supported (handled in main function)
        return null;
    }
</script>
...
<div class="flex-1 bg-[#101010] relative flex flex-col justify-center items-center overflow-hidden w-full h-full select-none">
    ...
    >
        <!-- Video Element (with Animation Preview) -->
        <video 
            bind:this={videoRef} 
            class="max-w-full max-h-full object-contain pointer-events-none 
                   {(isSourceMode && isVideoType($currentVideoSource.type)) || (!isSourceMode && activeClip && isVideoType(activeClip.type)) ? 'block' : 'hidden'}" 
            style={!isSourceMode && activeClip ? `
                transform: translate(${(activeClip.positionX || 0) * previewRatio}px, ${(activeClip.positionY || 0) * previewRatio}px) 
                           scale(${(activeClip.scale || 1) * activeClipAnim.scale}); 
                opacity: ${activeClipAnim.opacity};
                transform-origin: center;` : ''}
            muted={false} 
            crossorigin="anonymous"
        ></video>

        <!-- Image Element (with Animation Preview) -->
        <img 
            bind:this={imageRef}
            class="max-w-full max-h-full object-contain pointer-events-none 
                   {(isSourceMode && isImageType($currentVideoSource.type)) || (!isSourceMode && activeClip && isImageType(activeClip.type)) ? 'block' : 'hidden'}"
            style={!isSourceMode && activeClip ? `
                transform: translate(${(activeClip.positionX || 0) * previewRatio}px, ${(activeClip.positionY || 0) * previewRatio}px) 
                           scale(${(activeClip.scale || 1) * activeClipAnim.scale}); 
                opacity: ${activeClipAnim.opacity};
        ...
        <!-- Text Layer (with Animation Preview) -->
        {#if !isSourceMode && activeTextClip} 
            <div 
                ...
                style="
                    ...
                    transform: translate(-50%, -50%) scale({activeTextAnim.scale});
                    opacity: {activeTextAnim.opacity};
