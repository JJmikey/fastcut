<script>
    ...
    // Sync Video / Image / Background Blur
    $: if (!$isExporting && !isSourceMode) {
        if (activeClip) {
            ...
            if (bgVideoRef && bgVideoRef.src) bgVideoRef.removeAttribute('src');
            if (bgImageRef && bgImageRef.src) bgImageRef.removeAttribute('src');
        }
    ...
    $: if (!$isExporting) {
        if ($isPlaying && !isSourceMode) {
            if (videoRef && activeClip && isVideoType(activeClip.type)) {
                videoRef.play().catch(() => {});
                if ($projectSettings.backgroundMode === 'blur' && bgVideoRef) bgVideoRef.play().catch(()=>{});
            }
            if (audioRef && activeAudioClip) audioRef.play().catch(() => {});
        } else if (isSourceMode) {
        } else {
            if (videoRef) videoRef.pause();
            if (bgVideoRef) bgVideoRef.pause();
            if (audioRef) audioRef.pause();
        }
    }
    ...
    function addToProject() {
        ...
        const newClip = {
            ...
            animIn: 'none', animInDuration: 1.0,
            animOut: 'none', animOutDuration: 1.0
        };
    ...
    async function fastExportProcess() {
        ...
        try {
            ...
            for (let i = 0; i < totalFrames; i++) {
                ...
                else if ($projectSettings.backgroundMode === 'blur' && activeClip) {
                    let bgSource = null;
                    if (isVideoType(activeClip.type)) { bgSource = offscreenVideo; }
                    else if (isImageType(activeClip.type)) { bgSource = imageRef; }
                    if (bgSource) {
                        ctx.save(); ctx.filter = 'blur(40px) brightness(0.7)';
                        ctx.drawImage(bgSource, 0, 0, width, height); ctx.restore();
...
<div class="flex-1 bg-[#101010] relative flex flex-col justify-center items-center overflow-hidden w-full h-full select-none">
    ...
    <div 
        ...
        style="aspect-ratio: {$projectSettings.width} / {$projectSettings.height}; height: 80%; max-width: 90%;
               background-color: {$projectSettings.backgroundMode === 'color' ? $projectSettings.backgroundColor : 
    ...
    >
        ...
        {#if $projectSettings.backgroundMode === 'blur' && activeClip && (isVideoType(activeClip.type) || isImageType(activeClip.type))}
            <div class="absolute inset-0 overflow-hidden z-0">
                {#if isVideoType(activeClip.type)}
                    <video bind:this={bgVideoRef} class="w-full h-full object-cover blur-xl opacity-60 scale-110 pointer-events-none" muted></video>
                {:else}
                    <img bind:this={bgImageRef} class="w-full h-full object-cover blur-xl opacity-60 scale-110 pointer-events-none" alt="bg" />
                {/if}
            </div>
        {/if}

        <video bind:this={videoRef} class="max-w-full max-h-full object-contain pointer-events-none relative z-10 {(isSourceMode && isVideoType($currentVideoSource.type)) || (!isSourceMode && activeClip && isVideoType(activeClip.type)) ? 'block' : 'hidden'}" style={!isSourceMode && activeClip ? `transform: translate(${(activeClip.positionX || 0) * previewRatio}px, ${(activeClip.positionY || 0) * previewRatio}px) scale(${(activeClip.scale || 1) * activeClipAnim.scale}); opacity: ${activeClipAnim.opacity}; transform-origin: center;` : ''} muted={false} crossorigin="anonymous"></video>
        <img bind:this={imageRef} class="max-w-full max-h-full object-contain pointer-events-none relative z-10 {(isSourceMode && isImageType($currentVideoSource.type)) || (!isSourceMode && activeClip && isImageType(activeClip.type)) ? 'block' : 'hidden'}" style={!isSourceMode && activeClip ? `transform: translate(${(activeClip.positionX || 0) * previewRatio}px, ${(activeClip.positionY || 0) * previewRatio}px) scale(${(activeClip.scale || 1) * activeClipAnim.scale}); opacity: ${activeClipAnim.opacity}; transform-origin: center;` : ''} alt="preview" />
        
        {#if isSourceMode && isAudioType($currentVideoSource.type)} <div class="flex flex-col items-center gap-4 text-green-400 animate-pulse relative z-10"><svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg><span class="text-sm font-mono">Previewing Audio...</span></div> {/if}
        
        {#if !isSourceMode && activeTextClip} <div class="absolute pointer-events-none text-center select-none z-20" style="top: {activeTextClip.y}%; left: {activeTextClip.x}%; transform: translate(-50%, -50%) scale({activeTextAnim.scale}); opacity: {activeTextAnim.opacity}; font-size: {activeTextClip.fontSize * previewRatio}px; color: {activeTextClip.color}; font-family: {activeTextClip.fontFamily || 'Arial, sans-serif'}; font-weight: {activeTextClip.fontWeight || 'bold'}; white-space: pre; paint-order: stroke fill; -webkit-text-stroke: {activeTextClip.strokeWidth * previewRatio}px {activeTextClip.strokeColor}; background-color: {activeTextClip.showBackground ? activeTextClip.backgroundColor : 'transparent'}; padding: {activeTextClip.showBackground ? `${10 * previewRatio}px ${20 * previewRatio}px` : '0'}; border-radius: {8 * previewRatio}px; line-height: 1.2;">{activeTextClip.text}</div> {/if}
